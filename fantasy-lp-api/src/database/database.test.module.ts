    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (cfg: ConfigService) => {
        ...
        return {
          type: 'postgres',
          url,
          schema,                                           // ⬅️ fantasy → test
          synchronize: false,
          logging: ['error'],
          ssl: { rejectUnauthorized: false },
          extra: { options: `-c search_path=${schema},public` }, // ⬅️ orden correcto
          entities: [
            // Core (van a public por su decorador)
            League, Tournament, Role, Team, Player, Game, PlayerGameStats, TeamPlayerMembership,
            // Fantasy (van a "test" por schema de conexión)
