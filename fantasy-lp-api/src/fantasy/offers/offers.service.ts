  ...
  async respond(offerId: number, dto: RespondOfferDto) {
    return this.ds.transaction(async (trx) => {
      ...
      if (new Date(offer.expires_at) <= new Date()) {
        await trx.query(`UPDATE public.transfer_offer SET status='EXPIRED', updated_at=now() WHERE id=$1`, [offerId]);
        throw new BadRequestException('Oferta expirada');
      }

      if (!dto.accept) {
        await trx.query(`UPDATE public.transfer_offer SET status='REJECTED', updated_at=now() WHERE id=$1`, [offerId]);
      ...
      await trx.query(`UPDATE public.transfer_offer SET status='ACCEPTED', updated_at=now() WHERE id=$1`, [offerId]);
