...
import { TransferTransaction } from '../offers/transfer-transaction.entity';
...
export class MarketService {
  ...
  async createListing(dto: CreateListingDto) {
    // Verifica que el jugador está en ese equipo & liga
    ...
    const now = new Date();
    // Calcula cierre hoy a la hora de la liga (guardas UTC; aquí simplificamos usando hora fija sin TZ real)
  ...
  async placeBid(dto: PlaceBidDto, now = new Date()) {
    return this.ds.transaction(async (trx) => {
      ...
      const bidRows = await trx.query(
        `INSERT INTO public.market_bid (market_order_id, bidder_team_id, amount)
        VALUES ($1, $2, $3::bigint)
  ...
  async closeDailyAuctions(fantasyLeagueId: number, now = new Date()) {
    return this.ds.transaction(async (trx) => {
      ...
      for (const order of orders) {
        ...
        if (bids.length === 0) {
          await trx.query(`UPDATE public.market_order SET status='CLOSED', updated_at=now() WHERE id=$1`, [order.id]);
          continue;
        }
        ...
        for (const w of bids) {
          ...
          const existing = await trx.query(
            `SELECT id FROM public.fantasy_roster_slot
          ...
          const otherBidders: Array<{ bidder_team_id: number; last_amount: bigint }> = await trx.query(
            `SELECT DISTINCT b.bidder_team_id,
                    (SELECT lb.amount::bigint FROM public.market_bid lb
                      WHERE lb.market_order_id = b.market_order_id AND lb.bidder_team_id = b.bidder_team_id
                      ORDER BY lb.amount DESC, lb.created_at ASC LIMIT 1) AS last_amount
              FROM public.market_bid b
          ...
          await trx.query(`UPDATE public.market_order SET status='SETTLED', updated_at=now() WHERE id=$1`, [order.id]);
